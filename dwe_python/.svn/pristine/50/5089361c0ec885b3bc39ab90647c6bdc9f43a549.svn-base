...
def ber_array_calc(UI, det_jitter, ran_jitter, t_setup, t_hold):
    plot = np.zeros(int(UI*10**12))
    for i in range(int(UI*10**12)):
        plot[i] = cdf_jitter_calc_r(UI, i*10**(-12), det_jitter + t_setup, ran_jitter)
        plot[i] = plot[i] + cdf_jitter_calc_l(UI, i*10**(-12), det_jitter + t_hold, ran_jitter)
    return plot
...
def plot_bath(event):
    ...
    t_setup = ent_setup.get()
    t_hold  = ent_hold.get()
    
    ber_array = ber_array_calc(int(UI_input)*10**(-12), float(det_jitter_input)*10**(-12), float(ran_jitter_input)*10**(-12), float(t_setup)*10**(-12), float(t_hold)*10**(-12))
    ...
    plt.ylabel('BER')
    plt.ylim([10**-12,1])
    figure1 = plt.Figure(figsize=(5.5,4.5), dpi=100)
    ax1 = figure1.add_subplot(111)
    ax1.set_ylim([10**-12,1])
...
lab_setup = Label(root, text = "Время SETUP десериализатора, пс", font=("Helvetica", 12))
lab_setup.place(x = 200, y = 160)
ent_setup = Entry(root, width = 20, bd = 3)
ent_setup.place(x = 18, y = 160)

lab_hold = Label(root, text = "Время HOLD десериализатора, пс", font=("Helvetica", 12))
lab_hold.place(x = 200, y = 200)
ent_hold = Entry(root, width = 20, bd = 3)
ent_hold.place(x = 18, y = 200)
